...
let NGOs = [
    ...
    { _id: "ngo5", name: "ClotheTheNeedy", code: "NGO-CTN05", needs: "clothes, shoes, blankets", lat: 12.9750, lng: 77.6050, flagged: false, description: "Providing clothing to homeless and needy individuals" },
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('chatbot-send-btn').addEventListener('click', () => {
        ...
        if (recs.length > 0) {
            response = `I recommend: ${recs[0].ngo.name} (${recs[0].ngo.code}) - they need ${recs[0].ngo.needs}.`;
        }
    ...
    document.getElementById('auto-classify-btn').addEventListener('click', async () => {
        const file = document.getElementById('donor-image').files[0];
        if (!file) {
        ...
        try {
            ...
            document.getElementById('classify-status').innerHTML = `Predicted: ${label}<br>` + preds.slice(0, 3).map(p => `${p.className} — ${(p.probability * 100).toFixed(1)}%`).join('<br>');
    ...
    function populateDonorItems() {
        ...
        userDonations.forEach(d => {
            ...
            div.innerHTML = `
                <div class="flex-grow">
                    <p class="font-semibold">${d.item.name} (Qty: ${d.item.quantity}, ${d.item.category})</p>
                    <p class="text-sm">${d.item.description}</p>
                    <p class="text-sm">${d.ngoId ? `Sent to: ${NGOs.find(n => n._id === d.ngoId)?.name || 'Unknown'}` : 'Not assigned'}</p>
                </div>
                ${d.image ? `<img src="${d.image}" class="w-16 h-16 object-cover rounded-lg ml-4">` : ''}
            `;
    ...
    document.getElementById('add-item-btn').addEventListener('click', () => {
        ...
        if (!name || !quantity || !category || !description) {
            document.getElementById('donation-status').textContent = 'Please fill in all fields.';
            return;
        }
        const file = document.getElementById('donor-image').files[0];
        const imageUrl = file ? URL.createObjectURL(file) : null;
        donations.push({
            id: `donation_${donations.length + 1}`,
            donorId: currentUser.phone,
            item: { name, quantity, category, description },
            image: imageUrl,
            ngoId: null,
            usageImage: null,
        ...
        document.getElementById('donor-image').value = '';
        document.getElementById('donor-image-preview').innerHTML = '';
        document.getElementById('donation-status').textContent = 'Item added successfully.';
    });

    // Recommendations
    document.getElementById('recommend-btn').addEventListener('click', () => {
        const text = document.getElementById('donation-text').value.trim();
        if (!text) {
            document.getElementById('recommend-results').textContent = 'Please describe your donation.';
            return;
        }
        const recs = recommendNGOs(text, currentUser?.location);
        const results = document.getElementById('recommend-results');
        results.innerHTML = '';
        if (recs.length === 0) {
            results.textContent = 'No matching NGOs found.';
            return;
        }
        const select = document.getElementById('choose-ngo');
        select.innerHTML = '<option value="">Select an NGO</option>';
        recs.forEach(r => {
            const div = document.createElement('div');
            div.className = 'bg-gray-700 p-4 rounded-lg mb-2 hover:bg-gray-600 transition';
            div.innerHTML = `<p class="font-semibold">${r.ngo.name} (${r.ngo.code})</p><p class="text-sm">Needs: ${r.ngo.needs}${r.ngo.flagged ? ' <span class="text-red-400">(Flagged: Missing proof)</span>' : ''}</p>`;
    ...
    document.getElementById('create-donation-btn').addEventListener('click', () => {
        ...
        userDonations.forEach(d => d.ngoId = ngoId);
        flagNGOs();
        populateDonorItems();
    ...
    function populateAvailableDonations() {
        ...
        const ngoDonations = donations.filter(d => d.ngoId === currentUser.code);
        if (ngoDonations.length === 0) {
            donationsDiv.textContent = 'No donations available.';
            return;
        }
        ngoDonations.forEach(d => {
            ...
            div.innerHTML = `
                <div class="flex-grow">
                    <p class="font-semibold">${d.item.name} (Qty: ${d.item.quantity}, ${d.item.category})</p>
                    <p class="text-sm">${d.item.description}</p>
                    <p class="text-sm">${d.usageImage ? 'Proof submitted' : 'Awaiting proof'}</p>
                </div>
                ${d.image ? `<img src="${d.image}" class="w-16 h-16 object-cover rounded-lg ml-4">` : ''}
                <button class="claim-donation bg-blue-600 text-white px-4 py-2 rounded-lg ml-4 hover:bg-blue-700 transition" data-id="${d.id}">${d.status === 'claimed' ? 'Claimed' : 'Claim'}</button>
            `;
            donationsDiv.appendChild(div);
            if (!d.usageImage) {
                const opt = document.createElement('option');
                opt.value = d.id;
                opt.textContent = `${d.item.name} (Qty: ${d.item.quantity})`;
                select.appendChild(opt);
            }
        });
        document.querySelectorAll('.claim-donation').forEach(btn => {
    ...
    document.getElementById('ngo-usage-image').addEventListener('change', (ev) => {
        const preview = document.getElementById('ngo-usage-preview');
        preview.innerHTML = '';
        const file = ev.target.files[0];
        if (file) {
            const img = document.createElement('img');
    ...
    document.getElementById('submit-usage-btn').addEventListener('click', () => {
        const donationId = document.getElementById('ngo-donation-select').value;
        const file = document.getElementById('ngo-usage-image').files[0];
        if (!donationId || !file) {
            document.getElementById('usage-status').textContent = 'Please select a donation and upload a photo.';
            return;
        }
        const donation = donations.find(d => d.id === donationId);
        donation.usageImage = URL.createObjectURL(file);
        flagNGOs();
        populateAvailableDonations();
        document.getElementById('usage-status').textContent = 'Usage proof submitted.';
        document.getElementById('ngo-usage-image').value = '';
        document.getElementById('ngo-usage-preview').innerHTML = '';
        document.getElementById('ngo-donation-select').value = '';
    });

    document.getElementById('update-needs-btn').addEventListener('click', () => {
        const needs = document.getElementById('ngo-needs').value.trim();
        if (!needs) {
            document.getElementById('needs-status').textContent = 'Please enter your needs.';
            return;
        }
        const ngo = NGOs.find(n => n._id === currentUser.code);
        if (ngo) {
            ngo.needs = needs;
            document.getElementById('needs-status').textContent = 'Needs updated successfully.';
            CORPUS = buildCorpus(NGOs); // Rebuild corpus for recommendations
        }
    });
});